  ...
  try {
    ...
    if (s.rol !== "gerente") {
      document.body.innerHTML =
        "<h2 style='color:white; text-align:center; margin-top:50px;'>⛔ Acceso Restringido a Gerencia</h2>";
      return;
    }
...
async function cargarTabla() {
  ...
  if (!tbody) return;
  tbody.innerHTML =
    "<tr><td colspan='5' style='text-align:center'>⏳ Sincronizando datos...</td></tr>";
  ...
  if (error || !data) {
    tbody.innerHTML =
      "<tr><td colspan='5' style='text-align:center; color:#ef4444'>Error al cargar o sin datos.</td></tr>";
    return;
  }
...
function abrirModalCrear() {
  ...
  $("sel-usuario").disabled = false;
  $("sel-categoria").disabled = false;
  ...
  if (!asignacionEditandoID) {
    ...
    if (existente) {
      const confirmar = confirm(
        `⚠️ ${usuario} ya tiene asignada la categoría ${categoria}.\n¿Querés ACTUALIZAR la asignación existente?`
      );
      if (confirmar) asignacionEditandoID = existente.id;
      else return;
    }
  }
